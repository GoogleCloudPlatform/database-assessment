...
"""MySQL database container lifecycle management."""
...
class DatabaseConfig:
    """Configuration for MySQL database container."""

    # Container settings
    container_name: str = "cymbal_coffee_mysql-db-1"
    image: str = "mysql:8.0"
    hostname: str = "db"
    ...
    mysql_root_password: str = "super-secret"
    mysql_database: str = "app"
    mysql_user: str = "app"
    mysql_password: str = "super-secret"

    # Volumes
    data_volume_name: str = "mysql-db-data"
    ...
    def from_env(cls) -> DatabaseConfig:
        ...
        - DATABASE_ROOT_PASSWORD (default: super-secret)
        ...
        """
        return cls(
            host_port=int(os.getenv("DATABASE_PORT", "3306")),
            mysql_root_password=os.getenv("DATABASE_ROOT_PASSWORD", "super-secret"),
            mysql_database=os.getenv("DATABASE_NAME", "app"),
            mysql_user=os.getenv("DATABASE_USER", "app"),
            mysql_password=os.getenv("DATABASE_PASSWORD", "super-secret"),
        )
...
class MySQLDatabase:
    ...
    def start(
        ...
        *,
        ...
        self.console.rule("[bold blue]Starting MySQL Database Container")
        ...
        if self.runtime.container_running(self.config.container_name):
            if not recreate:
                ...
                raise ContainerAlreadyRunningError(error_msg)
        ...
        # Start container
        self.console.print("[cyan]Starting MySQL container...[/cyan]")
    ...
    def _build_run_command(self) -> list[str]:
        ...
        return [
            ...
            "-e",
            f"MYSQL_ROOT_PASSWORD={self.config.mysql_root_password}",
            "-e",
            f"MYSQL_DATABASE={self.config.mysql_database}",
            "-e",
            f"MYSQL_USER={self.config.mysql_user}",
            "-e",
            f"MYSQL_PASSWORD={self.config.mysql_password}",
            "-v",
            f"{self.config.data_volume_name}:/var/lib/mysql",
            "--restart",
    ...
    def _wait_for_health(self) -> None:
            ...
            try:
                ...
                if status.get("status") == "running":
                    ...
                    if health_status == "unhealthy":
                        error_msg = "Container became unhealthy"
        ...
        error_msg = f"Database did not become healthy within {max_wait} seconds"
        raise ContainerStartError(error_msg)
    ...
    def _display_connection_info(self) -> None:
        ...
        self.console.print(f"  Database: [cyan]{self.config.mysql_database}[/cyan]")
        self.console.print(f"  User: [cyan]{self.config.mysql_user}[/cyan]")
        self.console.print(f"  Password: [cyan]{self.config.mysql_password}[/cyan]")
        self.console.print("\n[bold]Connection String:[/bold]")
        self.console.print(
            f"  [cyan]mysql://{self.config.mysql_user}:{self.config.mysql_password}@localhost:{self.config.host_port}/{self.config.mysql_database}[/cyan]"
        )
    ...
    def stop(self) -> None:
        ...
        self.console.rule("[bold blue]Stopping MySQL Database")
    ...
    def restart(self) -> None:
        ...
        self.console.rule("[bold blue]Restarting MySQL Database")
