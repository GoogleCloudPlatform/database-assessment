...
"""Health checker for MySQL deployment."""
...
class HealthChecker:
    """Check health of MySQL deployment components."""
    ...
    def check_container(self) -> ComponentHealth:
        ...
        """
        from tools.mysql.database import DatabaseConfig
        ...
        if not self.runtime.container_exists(config.container_name):
            return ComponentHealth(
                ...
                suggestions=["Start container: python manage.py database mysql start"],
            )
    ...
    @staticmethod
    def check_mysql_cli() -> ComponentHealth:
        """Check mysql CLI availability.

        Returns:
            ComponentHealth: mysql status
        """
        try:
            mysql_path = shutil.which("mysql")
            if not mysql_path:
                msg = "mysql executable not found"
            ...
            if result.returncode == 0:
                ...
                return ComponentHealth(
                    name="mysql CLI",
                    status=HealthStatus.HEALTHY,
                    message=f"mysql is available: {version}",
            ...
            return ComponentHealth(
                ...
                message="mysql not available (optional)",
                suggestions=["Install MySQL client tools for mysql access"],
            )
    ...
    def check_connectivity(self) -> ComponentHealth:
        ...
        # Try mysqladmin ping via docker/podman exec
        try:
            ...
            return ComponentHealth(
                ...
                suggestions=["Check database logs: python manage.py database mysql logs"],
            )
