"""CLI commands for SQL Server database container management."""
...
def start_database(pull: bool, recreate: bool) -> None:
    ...
    try:
        ...
        db = SQLServerDatabase(runtime=runtime, config=config, console=console)
...
def stop_database(timeout: int) -> None:
    """Stop SQL Server database container."""
    try:
        runtime = ContainerRuntime()
        config = DatabaseConfig.from_env()
        db = SQLServerDatabase(runtime=runtime, config=config, console=console)
        ...
        console.print("[yellow]Stopping SQL Server database container...[/yellow]")
        db.stop()
...
def restart_database(timeout: int) -> None:
    """Restart SQL Server database container."""
    try:
        runtime = ContainerRuntime()
        config = DatabaseConfig.from_env()
        db = SQLServerDatabase(runtime=runtime, config=config, console=console)

        console.print("[yellow]Restarting SQL Server database container...[/yellow]")
        db.restart()
...
def status_database(verbose: bool) -> None:
    """Show SQL Server database container status."""
    try:
        runtime = ContainerRuntime()
        config = DatabaseConfig.from_env()
        db = SQLServerDatabase(runtime=runtime, config=config, console=console)
...
def logs_database(follow: bool, tail: int) -> None:
    """Display SQL Server database container logs."""
    try:
        runtime = ContainerRuntime()
        config = DatabaseConfig.from_env()
        db = SQLServerDatabase(runtime=runtime, config=config, console=console)
...
def remove_database(force: bool) -> None:
    """Remove SQL Server database container."""
    try:
        runtime = ContainerRuntime()
        config = DatabaseConfig.from_env()
        db = SQLServerDatabase(runtime=runtime, config=config, console=console)

        console.print("[yellow]Removing SQL Server database container...[/yellow]")
...
def test_connection(timeout: int) -> None:
    ...
    from tools.sqlserver.connection import ConnectionConfig, ConnectionTester
...
def connection_info() -> None:
    ...
    from tools.sqlserver.connection import ConnectionTester
