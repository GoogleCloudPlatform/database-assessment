...
column cdb_name new_value v_cdbname noprint
column pdb_name new_value v_pdbname noprint
...
column db_type new_value v_db_type noprint
...
SELECT db_unique_name cdb_name
      , sys_context('USERENV','CON_ID') con_id
      , decode(sys_context('USERENV', 'CON_NAME'),'CDB$ROOT',sys_context('USERENV', 'DB_NAME'),sys_context('USERENV', 'CON_NAME')) pdb_name
      , decode(sys_context('USERENV','CON_ID'),1,'CDB','PDB') db_type
FROM   v$database
/
...
BEGIN
 IF :db_version = '19C' and :v_awr_license = '19C' then
        select  'spool opdb__awrsnapdetails__&&v_tag' || chr(10) || 'SELECT banner from v$version ;' || chr(10) || 'spool off'
             INTO :v_sql from dual;
 ELSE
        select  'spool opdb__awrsnapdetails__&&v_tag' || chr(10) || 
                        'SELECT version FROM dba_feature_usage_statistics
                where name like ''Automatic Workload%'' or name like ''AWR%'';' || chr(10) || 'spool off' 
            INTO :v_sql from dual;
 END IF;
END;
/
...
/* File opdb__awrsnapdetails__ */

BEGIN
 IF :db_version = '19C' and :v_awr_license = '19C' then
        SELECT  '
spool spool opdb__awrsnapdetails__&v_tag
...
spool select.sql append
print :v_sql
spool off

/* File opdb__dbsummary__ */
...
SELECT ''&&v_host''
       ...
       (SELECT protection_level
        FROM   v$database)                                                      AS dg_protection_level
FROM   dual)
SELECT pkey || ''&&colspr'' || dbid || ''&&colspr'' || db_name || ''&&colspr'' || ''N/A'' || ''&&colspr'' || dbversion || ''&&colspr'' || dbfullversion || ''&&colspr'' || log_mode || ''&&colspr'' || force_logging || ''&&colspr'' ||
       redo_gb_per_day || ''&&colspr'' || rac_dbinstaces || ''&&colspr'' || characterset || ''&&colspr'' || platform_name || ''&&colspr'' || startup_time || ''&&colspr'' || user_schemas || ''&&colspr'' ||
	   buffer_cache_mb || ''&&colspr'' || shared_pool_mb || ''&&colspr'' || total_pga_allocated_mb || ''&&colspr'' || db_size_allocated_gb || ''&&colspr'' || db_size_in_use_gb || ''&&colspr'' ||
	   db_long_size_gb || ''&&colspr'' || dg_database_role || ''&&colspr'' || dg_protection_mode || ''&&colspr'' || dg_protection_level
FROM vdbsummary;
...
SELECT ''&&v_host''
       ...
       GROUP  BY ''&&v_host''
              ...
              || ''&&v_hora'',
              owner, segment_type )
SELECT pkey || ''&&colspr'' || ''N/A'' || ''&&colspr'' || owner || ''&&colspr'' || segment_type || ''&&colspr'' || GB
FROM vused;
...
WITH vtbcompress AS (
        ...
        HAVING TRUNC(SUM(table_gbytes) + SUM(partition_gbytes)
                     + SUM(subpartition_gbytes)) > 0)
SELECT pkey || ''&&colspr'' || ''N/A'' || ''&&colspr'' || owner || ''&&colspr'' || tab || ''&&colspr'' || table_gb || ''&&colspr'' || part || ''&&colspr'' || part_gb || ''&&colspr'' ||
       subpart || ''&&colspr'' || subpart_gb || ''&&colspr'' || total_gbytes
FROM vtbcompress
ORDER  BY total_gbytes DESC;
...
WITH vcompresstype AS (
    SELECT ''&&v_host''
            ...
            TRUNC(SUM(DECODE(compress_for, ''ARCHIVE LOW'', gbytes,
                                        0))) archive_low,
            TRUNC(SUM(DECODE(compress_for, 'A'RCHIVE HIGH'', gbytes,
                                        0))) archive_high,
...
WITH vcompresstype AS (
        ...
        GROUP  BY owner
        HAVING TRUNC(SUM(gbytes)) > 0)
SELECT pkey || ''&&colspr'' || ''N/A'' || ''&&colspr'' || owner || ''&&colspr'' || basic || ''&&colspr'' || oltp || ''&&colspr'' || query_low || ''&&colspr'' || query_high || ''&&colspr'' ||
       archive_low || ''&&colspr'' || archive_high || ''&&colspr'' || total_gb
FROM vcompresstype
ORDER BY total_gb DESC;
...
GROUP BY ''&&v_host''
       ...
       || ''&&v_hora'',
       owner)
SELECT pkey || ''&&colspr'' || ''N/A'' || ''&&colspr'' || owner || ''&&colspr'' || count
FROM vdbl;
...
FROM   gv$parameter
ORDER  BY 2)
SELECT pkey || ''&&colspr'' || inst_id || ''&&colspr'' || ''N/A'' || ''&&colspr'' || name || ''&&colspr'' || value || ''&&colspr'' || ''N/A'' || ''&&colspr'' || isdefault
FROM vparam;
...
ORDER  BY name)
SELECT pkey || ''&&colspr'' || ''N/A'' || ''&&colspr'' || name || ''&&colspr'' || currently_used || ''&&colspr'' || detected_usages || ''&&colspr'' ||
       total_samples || ''&&colspr'' || first_usage || ''&&colspr'' || last_usage || ''&&colspr'' || aux_count
FROM vdbf;
...
/* File opdb__dbhwmarkstatistics__ */
...
WITH vdbobj AS (
        ...
        GROUP  BY grouping sets ( ( object_type ),
		                          ( owner, object_type ) ))
SELECT pkey || ''&&colspr'' || ''N/A'' || ''&&colspr'' || owner || ''&&colspr'' || object_type || ''&&colspr'' || ''N/A'' || ''&&colspr'' ||
       count || ''&&colspr'' || ''N/A'' || ''&&colspr'' || in_owner || ''&&colspr'' || in_object_type || ''&&colspr'' || ''N/A''
FROM vdbobj;
...
GROUP  BY pkey,
          owner,
          TYPE)
SELECT pkey || ''&&colspr'' || ''N/A'' || ''&&colspr'' || owner || ''&&colspr'' || type || ''&&colspr'' || sum_nr_lines || ''&&colspr'' || qt_objs || ''&&colspr'' || sum_nr_lines_w_utl || ''&&colspr'' ||
       sum_nr_lines_w_dbms || ''&&colspr'' || count_exec_im || ''&&colspr'' || count_dbms_sql || ''&&colspr'' || sum_nr_lines_w_dbms_utl || ''&&colspr'' || sum_count_total
FROM vsrc;
...
GROUP  BY ''&&v_host''
          ...
          owner,
          index_type)
SELECT pkey || ''&&colspr'' || ''N/A''  || ''&&colspr'' || owner || ''&&colspr'' || index_type || ''&&colspr'' || cnt
FROM vidxtype;
...
GROUP  BY ''&&v_host''
          ...
          owner,
          data_type)
SELECT pkey || ''&&colspr'' || ''N/A'' || ''&&colspr'' || owner || ''&&colspr'' || data_type || ''&&colspr'' || cnt
FROM vdtype;
spool off
...
GROUP  BY ''&&v_host''
          ...
          || ''&&v_hora'',
          owner)
SELECT pkey || ''&&colspr'' || ''N/A'' || ''&&colspr'' || owner || ''&&colspr'' || pk || ''&&colspr'' || uk || ''&&colspr'' || ck || ''&&colspr'' ||
       ri || ''&&colspr'' || vwck || ''&&colspr'' || vwro || ''&&colspr'' || hashexpr || ''&&colspr'' || suplog || ''&&colspr'' || num_tables || ''&&colspr'' || total_cons
FROM vnopk;
...
/* File opdb__awrhistsysmetrichist__ */
...
/* File opdb__awrhistsysmetricsumm__ */
...
GROUP  BY ''&&v_host''
          ...
          TO_CHAR(c.begin_interval_time, ''hh24''),
          b.command_type)
SELECT pkey || ''&&colspr'' || ''N/A'' || ''&&colspr'' || hh24 || ''&&colspr'' || command_type || ''&&colspr'' || cnt || ''&&colspr'' || avg_buffer_gets || ''&&colspr'' || avg_elasped_time || ''&&colspr'' ||
       avg_rows_processed || ''&&colspr'' || avg_executions || ''&&colspr'' || avg_cpu_time || ''&&colspr'' || avg_iowait || ''&&colspr'' || avg_clwait || ''&&colspr'' ||
	   avg_apwait || ''&&colspr'' || avg_ccwait || ''&&colspr'' || avg_plsexec_time
FROM vcmdtype;
...
/* File opdb__dbahistsystimemodel__ */
...
(SELECT name
 FROM system.logstdby$skip_support
 WHERE action=0))
 SELECT pkey || ''&&colspr'' || ''N/A'' || ''&&colspr'' || owner || ''&&colspr'' || segment_name || ''&&colspr'' || segment_type || ''&&colspr'' || tablespace_name
FROM vuseg;
...
SELECT pkey || ''&&colspr'' || ''N/A'' || ''&&colspr'' || owner || ''&&colspr'' || table_name || ''&&colspr'' || type_owner || ''&&colspr'' || type_name || ''&&colspr'' ||
       default_directory_owner || ''&&colspr'' || default_directory_name
FROM vexttab;
...
ORDER BY elapsed_time_total DESC)
SELECT pkey || ''&&colspr'' || ''N/A'' || ''&&colspr'' || dbid || ''&&colspr'' || instance_number || ''&&colspr'' || force_matching_signature || ''&&colspr'' || sql_id || ''&&colspr'' ||
       total_executions || ''&&colspr'' || total_px_servers_execs || ''&&colspr'' || elapsed_time_total || ''&&colspr'' || disk_reads_total || ''&&colspr'' ||
       physical_read_bytes_total || ''&&colspr'' || physical_write_bytes_total || ''&&colspr'' || io_offload_elig_bytes_total || ''&&colspr'' || io_interconnect_bytes_total || ''&&colspr'' ||
       optimized_physical_reads_total || ''&&colspr'' || cell_uncompressed_bytes_total || ''&&colspr'' || io_offload_return_bytes_total || ''&&colspr'' || direct_writes_total || ''&&colspr'' ||
       perc_exec_finished || ''&&colspr'' || avg_rows || ''&&colspr'' || avg_disk_reads || ''&&colspr'' || avg_buffer_gets || ''&&colspr'' || avg_cpu_time_us || ''&&colspr'' || avg_elapsed_us || ''&&colspr'' || avg_iowait_us || ''&&colspr'' ||
...
SELECT ''&&v_host''
       ...
       || ''_''
       || ''&&v_hora'' AS pkey,
       table_owner, table_name, count(1) idx_cnt
FROM dba_indexes
WHERE  owner NOT IN
                    ( SELECT name
                      FROM   SYSTEM.logstdby$skip_support
...
SELECT pkey,
       ...
       round(100*ratio_to_report(count(table_name)) over ()) idx_perc
FROM vrawidx
GROUP BY pkey, idx_cnt)
SELECT pkey || ''&&colspr'' || ''N/A'' || ''&&colspr'' || tab_count || ''&&colspr'' || idx_cnt || ''&&colspr'' || idx_perc
FROM vcidx;
