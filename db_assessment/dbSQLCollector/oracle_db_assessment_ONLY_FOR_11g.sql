...
SELECT  '&&v_host'
        ...
        || '_'
        || '&&v_hora' AS pkey,
        inst_id, dest_id, dest_name, destination, status, target, schedule, register,
...
SELECT pkey || '&&colspr' || 'N/A' || '&&colspr' || inst_id || '&&colspr' || log_archive_config || '&&colspr' || dest_id || '&&colspr' || dest_name || '&&colspr' || destination || '&&colspr' || status || '&&colspr' ||
