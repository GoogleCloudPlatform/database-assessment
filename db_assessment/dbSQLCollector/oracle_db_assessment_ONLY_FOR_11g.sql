...
SELECT pkey || '&&colspr' || 'N/A' || '&&colspr' || owner || '&&colspr' || segment_type || '&&colspr' || tablespace_name || '&&colspr' || flash_cache || '&&colspr' ||
       'N/A' || '&&colspr' || 'N/A' || '&&colspr' || IN_OWNER || '&&colspr' || IN_SEGMENT_TYPE || '&&colspr' || IN_TABLESPACE_NAME || '&&colspr' || IN_FLASH_CACHE || '&&colspr' || GB
FROM vused;
...
SELECT pkey || '&&colspr' || 'N/A' || '&&colspr' || owner || '&&colspr' || segment_type || '&&colspr' || total_gb
FROM vspaceow
ORDER  BY total_gb DESC;
...
SELECT pkey || '&&colspr' || 'N/A' ||' , ' || tablespace_name || '&&colspr' || status || '&&colspr' || total_gb || '&&colspr' ||
...
WHERE  owner NOT IN
                     ...
                     FROM   SYSTEM.logstdby$skip_support
                     WHERE  action=0))
SELECT pkey || '&&colspr' || 'N/A' || '&&colspr' || owner || '&&colspr' || db_link || '&&colspr' || host || '&&colspr' || created
FROM vdbl;
...
GROUP  BY '&&v_host'
          ...
          partitioning_type,
          subpartitioning_type)
SELECT pkey || '&&colspr' || 'N/A' || '&&colspr' || owner || '&&colspr' || partitioning_type || '&&colspr' || subpartitioning_type || '&&colspr' || cnt
FROM vpart;
...
SELECT pkey || '&&colspr' || '11g' || '&&colspr' || time || '&&colspr' || action || '&&colspr' || namespace || '&&colspr' || version || '&&colspr' || id || '&&colspr' || comments
FROM vpatch;
...
SELECT pkey || '&&colspr' || MESSAGE_TIME || '&&colspr' || message_text || '&&colspr' || host_id || '&&colspr' || 'N/A' || '&&colspr' || component_id || '&&colspr' ||
       message_type || '&&colspr' || message_level || '&&colspr' || message_id || '&&colspr' || message_group
FROM valert
WHERE  ROWNUM < 5001;
...
FROM dba_services
ORDER BY NAME)
SELECT pkey || '&&colspr' || 'N/A' || '&&colspr' || 'N/A' || '&&colspr' || service_id || '&&colspr' || service_name || '&&colspr' || network_name || '&&colspr' || creation_date || '&&colspr' ||
...
SELECT '&&v_host'
       ...
       || '_'
       || '&&v_hora' AS pkey,
       table_owner, table_name, table_type, index_type, count(1) idx_cnt
FROM dba_indexes
WHERE  owner NOT IN
                    ( SELECT name
                      FROM   SYSTEM.logstdby$skip_support
                      WHERE  action=0)
group by table_owner, table_name, table_type, index_type),
vcidx AS (
SELECT pkey,
