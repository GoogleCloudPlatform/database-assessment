...
do
 ./make_dist.sh --target "${line}" &
 # Wait a couple of seconds before starting another build.
 sleep 2
 
 # Do not run another biuld if there are too many running already
 while [[ $(ps -ef | grep make_dist.sh | grep -v grep | grep target | wc -l) -ge ${MAXPARALLEL} ]]
 do
  echo sleeping for 10 secs while waiting on builds:
  ps | grep make_dist.sh | grep -v grep 
