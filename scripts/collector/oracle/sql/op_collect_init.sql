...
BEGIN 
  ...
  ELSE
     ...
     :v_stats_source := ' without performance data';
  END IF;
  :v_info_prompt := 'About to collect data for database &v_dbname &&v_dbid ' || :v_info_prompt ;
  :v_end_of_job_summary := 'Collected data for database &v_dbname ID &&v_dbid ' || :v_stats_source ;
END;
/
set termout off
