...
SELECT :v_pkey                                                                                                                 AS pkey,
       ...
       (SELECT &s_db_container_col. FROM v$database)                                                                           AS cdb,
       (SELECT version FROM v$instance)                                                                                        AS db_version,
       (SELECT REPLACE( &s_banner_ver_col. , CHR(10), ' ') as banner FROM v$version where banner LIKE '%Database%' )           AS db_fullversion,
       (SELECT log_mode FROM v$database)                                                                                       AS log_mode,
       (SELECT force_logging FROM v$database)                                                                                  AS force_logging,
       (SELECT COUNT(1) FROM gv$instance)                                                                                      AS rac_dbinstances,
       (SELECT value
       ...
       (SELECT &s_platform_name. as  platform_name FROM v$database)                                                            AS platform_name,
       (SELECT TO_CHAR(startup_time, 'YYYY-MM-DD HH24:MI:SS') FROM v$instance)                                                 AS startup_time,
       (SELECT COUNT(1)
        FROM   &s_tblprefix._users
        WHERE  username NOT IN
@sql/extracts/exclude_schemas.sql
       )                                                                                                                       AS user_schemas,
       (SELECT ROUND(SUM(bytes / 1024 / 1024)) FROM   v$sgastat WHERE  name = 'buffer_cache')                                  AS buffer_cache_mb,
       (SELECT ROUND(SUM(bytes / 1024 / 1024)) FROM   v$sgastat WHERE  pool = 'shared pool')                                   AS shared_pool_mb,
       (SELECT ROUND(value / 1024 / 1024, 0) FROM   v$pgastat WHERE  name = 'total PGA allocated')                             AS total_pga_allocated_mb,
       (SELECT ( ROUND(SUM(bytes) / 1024 / 1024 / 1024) ) FROM &s_tblprefix._data_files)                                       AS db_size_allocated_gb,
       (SELECT ( ROUND(SUM(bytes) / 1024 / 1024 / 1024) ) FROM &s_tblprefix._segments WHERE  owner NOT IN ( 'SYS', 'SYSTEM' )) AS db_size_in_use_gb,
       ...
       (SELECT ( ROUND(SUM(bytes) / 1024 / 1024 / 1024) ) FROM   &s_tblprefix._temp_files)                                     AS db_size_temp_allocated_gb,
