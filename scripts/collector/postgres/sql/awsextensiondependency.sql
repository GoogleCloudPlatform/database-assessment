  ...
  (WITH alias1 AS
      ...
      INNER JOIN
        (SELECT DISTINCT pg_proc.oid AS procoid,
         ...
         WHERE nspname = 'aws_oracle_ext'
           AND pg_proc.pronamespace = pg_namespace.oid ) alias1 ON pg_depend.refobjid = alias1.procoid
    ...
    JOIN LATERAL
        (SELECT i AS funcname,
                cntgroup AS cnt
         FROM
           (SELECT (regexp_matches(alias1.def, 'aws_oracle_ext[.][a-z]*[_,a-z,$,"]*', 'ig')) [1] i,
                                                                                                 count(1) cntgroup
            GROUP BY (regexp_matches(alias1.def, 'aws_oracle_ext[.][a-z]*[_,a-z,$,"]*', 'ig'))[1]) t) AS alias2
      WHERE def ~*'aws_oracle_ext.*' ) SELECT alias2.schema AS schemaName,
                                              'N/A' AS LANGUAGE,
        ...
        (SELECT i AS funcname,
         ...
         FROM
           (SELECT (regexp_matches(alias1.def, 'aws_oracle_ext[.][a-z]*[_,a-z,$,"]*', 'ig'))[1] i,
                                                                                                count(1) cntgroup
            GROUP BY (regexp_matches(alias1.def, 'aws_oracle_ext[.][a-z]*[_,a-z,$,"]*', 'ig'))[1]) t) AS alias2
      WHERE def ~*'aws_oracle_ext.*' ) SELECT alias2.table_schema AS schemaName,
                                              'N/A' AS LANGUAGE,
...
UNION
  (WITH alias1 AS
     (SELECT DISTINCT n.nspname AS function_schema,
        ...
        (SELECT 'Y'
         FROM pg_trigger
         WHERE tgfoid = (n.nspname || '.' || p.proname) :: regproc ) AS Trigger_Func,
  ...
  JOIN pg_type t ON t.oid = p.prorettype
     ...
     (SELECT alias1.function_schema,
      ...
      WHERE def ~*'aws_oracle_ext.*'
        AND Trigger_Func = 'Y' ) SELECT function_schema AS schemaName,
        ...
        (SELECT i AS funcname,
         ...
         FROM
           (SELECT (regexp_matches(alias1.def, 'aws_oracle_ext[.][a-z]*[_,a-z,$,"]*', 'ig'))[1] i,
            ...
            GROUP BY (regexp_matches(alias1.def, 'aws_oracle_ext[.][a-z]*[_,a-z,$,"]*', 'ig'))[1]) t) AS alias2
      WHERE def ~*'aws_oracle_ext.*'
        AND alias1.Trigger_Func IS NULL ) SELECT function_schema AS schemaName,
                                                 function_language AS LANGUAGE,
                                                 'Functions' AS TYPE,
                                                 function_name AS typeName,
                                                 funcname AS AWSExtensionDependency,
        ...
        (SELECT i AS funcname,
         ...
         FROM
           (SELECT (regexp_matches(alias1.def, 'aws_oracle_ext[.][a-z]*[_,a-z,$,"]*', 'ig'))[1] i,
                                                                                                count(1) cntgroup
            GROUP BY (regexp_matches(alias1.def, 'aws_oracle_ext[.][a-z]*[_,a-z,$,"]*', 'ig'))[1]) t) AS alias2
      WHERE def ~*'aws_oracle_ext.*' ) SELECT function_schema AS schemaName,
                                              function_language AS LANGUAGE,
                                              'Procedures' AS TYPE,
                                              function_name AS typeName,
                                              funcname AS AWSExtensionDependency,
