...
BEGIN
	...
	BEGIN TRY
	IF @PRODUCT_VERSION > 12 AND @validDB <> 0 AND @CLOUDTYPE = 'NONE' AND @ASSESSMENT_DATABSE_NAME <> 'tempdb'
	BEGIN
		exec ('
				WITH TableData AS (
					SELECT 
	...
	BEGIN
		exec ('
			WITH TableData AS (
				...
				FROM TableData');
	END;
	IF @PRODUCT_VERSION >= 12 AND @validDB <> 0 AND @CLOUDTYPE = 'AZURE' AND @ASSESSMENT_DATABSE_NAME <> 'tempdb'
    BEGIN
		exec ('
		WITH TableData AS (
			SELECT 
