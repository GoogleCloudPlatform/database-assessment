...
SELECT @CLOUDTYPE = 'AZURE' BEGIN IF EXISTS (
        SELECT name
        FROM sys.server_principals
        WHERE name = @COLLECTION_USER
    ) IF @CLOUDTYPE = 'AZURE' BEGIN TRY exec (
...
END CATCH IF @CLOUDTYPE <> 'AZURE' BEGIN TRY exec (
...
END CATCH IF @PRODUCT_VERSION > 11 BEGIN TRY exec (
...
END CATCH IF @PRODUCT_VERSION > 15 BEGIN TRY exec(
...
IF @CLOUDTYPE <> 'AZURE' OPEN db_cursor FETCH NEXT
FROM db_cursor INTO @dbname WHILE @@FETCH_STATUS = 0 BEGIN BEGIN TRY exec (
        '
            use [' + @dbname + '];
...
CLOSE db_cursor DEALLOCATE db_cursor IF @CLOUDTYPE = 'AZURE' BEGIN exec (
