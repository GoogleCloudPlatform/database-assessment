...
def prepare_pyapp_env(options: argparse.Namespace) -> None:
    Path("./build").mkdir(exist_ok=True)
    subprocess.run(["/usr/bin/env", "wget", PYAPP_URL, "-O", "./build/pyapp.tar.gz"], check=False)
    subprocess.run(
        [
            "/usr/bin/env",
            "tar",
