...
def version_check(console: Console, db: duckdb.DuckDBPyConnection, queries: Queries) -> None:
    ...
    if is_rds:
        ...
        if rds_min_version_check_fail(version, major):
            queries.insert_readiness_check(
                ...
                info=f"Source RDS database server has unsupported minor version: ({version})",
            )
    ...
    if major not in db_type_map or major < min_major_version:
        queries.insert_readiness_check(
            ...
            severity="ERROR",
            assessment_type="INCOMPATIBLE_DATABASE_VERSION",
            info=f"Replication from source database server ({version}) is not supported",
        )


def collation_check(console: Console, db: duckdb.DuckDBPyConnection, queries: Queries) -> None:
...
def execute_postgres_assessment(
    console: Console, local_db: duckdb.DuckDBPyConnection, manager: CanonicalQueryManager
) -> None:
