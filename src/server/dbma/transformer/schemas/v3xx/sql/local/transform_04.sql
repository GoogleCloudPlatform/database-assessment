...
create
or replace table T_DS_BMS_sizing as WITH hostdetails AS (
    SELECT
...
DBStorage AS (
    ...
    from
        dbsizing_facts AS dbsizing_facts
...
DBIO as(
    select
        --Step 2 - Find the max IOPS and MBps across the 24 hours at the 95% percentile
        pkey,
    ...
    FROM
        (
            SELECT
                -- Step 1 -find the SUM of IOPS and MBps by database for each hour
                sysstat.pkey,
                dbid,
                hour,
                ROUND(
            ...
            FROM
                vsysstat_columnar as sysstat --  JOIN vdbinstances as dbinstance
...
DBHostName as(
    select
        --Determine the host_names of the on-premises servers
        DISTINCT sysstat.pkey,
        host_name
    from
        vsysstat_columnar as sysstat -- WAP Does not need to be this view?
        JOIN dbinstances as dbinstance ON sysstat.PKEY = dbinstance.PKEY
        AND sysstat.instance_number = dbinstance.inst_id
),
DBHostJoin as(
    select
        --Join the on-premises hosts to the databases
        DBIO.*,
...
DBIOMissing AS (
    SELECT
        --Find which on-premises hosts are part of a RAC database
        DISTINCT host_name
    FROM
        DBHostName
    EXCEPT
...
DBIOFinalResults AS (
    SELECT
        --Sum the results of the RAC databases for each host
        host_name,
...
DBIO_sizing as(
    ...
    from
        DBIOFinalResults as DBIOFinalResults
